...
def move_motor(direction, total_steps, max_speed=500, accel_steps=100):
    ...
    for step in range(total_steps):
        phase_progress = (
            step / accel_steps
            if step < accel_steps
            else (total_steps - step) / accel_steps
        )

        # Adjust acceleration based on an S-curve profile
        accel_factor = (
            1 - math.cos(math.pi * phase_progress)
        ) / 2  # Generates an S-curve shape factor
...
def align():
    consecutive_aligned = (
        0  # Counter for how many times the target is consecutively aligned
    )
    ...
    while consecutive_aligned <= REQ_CONSEC:
        ...
        if x_displacement is not None:
            ...
            if steps_needed != 0:
                ...
                print(
                    f"Moved {abs(steps_needed)} steps {'right' if direction else 'left'} to align."
                )
        else:
