...
class TargetDetector:
    def __init__(
        ...
        self.cap: cv2.VideoCapture = self.initialize_camera(
    ...
    def initialize_camera(self, camera_index, width, height) -> cv2.VideoCapture:
        cap: cv2.VideoCapture = cv2.VideoCapture(camera_index)
        if not cap.isOpened():
            raise IOError("Cannot open webcam")
        # TODO: Uncomment the following lines will report an error
        # In line 30 (if not ret: raise IOError("Cannot read from webcam"))
